...
project_dir <- normalizePath(dirname(sub("^--file=", "", commandArgs(trailingOnly = FALSE)[grep("^--file=", commandArgs(trailingOnly = FALSE))][1])), winslash = "/")
...
source(file.path(project_dir, "R", "full", "05_prs_associations.R"))
...
if (mode == "help") {
  cat(paste0(
    ...
    "GWAS preparation and chromosome scoring are dispatched by tools/run_yu_prs_windows.ps1.\n"
  ))
  quit(status = 0)
}
