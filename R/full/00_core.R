...
yur_abs_path <- function(path, dir0, project_dir = NULL) {
  ...
  project_relative <- startsWith(path, "config/") || startsWith(path, "references/")
  root <- if (!is.null(project_dir) && project_relative) project_dir else dir0
  normalizePath(file.path(root, path), winslash = "/", mustWork = FALSE)
}

yur_init_config <- function(project_dir, cli) {
  cfg_file <- file.path(project_dir, "config", "full_reproduction_defaults.json")
  ...
  cfg$outcomes_file <- file.path(cfg$project_dir, "config", "outcomes.csv")
  cfg$field_map_file <- file.path(cfg$project_dir, "config", "local_field_map.csv")
  cfg$method_provenance_file <- file.path(cfg$project_dir, "config", "method_provenance.csv")
  cfg$cmr_metrics_file <- file.path(cfg$project_dir, "config", "cmr_metrics.csv")
