...
yur_evaluate_prediction <- function(cfg) {
  ...
  cad_reference_file <- file.path(cfg$project_dir, "config", "yu_cad_published_metrics.csv")
...
yur_build_figures <- function(cfg) {
  ...
  if (!is.null(prs)) {
    ...
    threshold_contract <- fread(file.path(cfg$project_dir, "config", "prs_thresholds.tsv"))
