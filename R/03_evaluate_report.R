...
yuy_evaluate <- function(cfg) {
  ...
  pubfile<-file.path(cfg$script_dir,"config","yu_cad_published_metrics.csv"); if(file.exists(pubfile)) yuy_write_csv(fread(pubfile),file.path(cfg$paths$evaluation,"published_cad_reference_metrics.csv"))
