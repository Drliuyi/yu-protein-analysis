...
$RFile=Join-Path $ProjectDir "99_run_yu_yys.R"
$Config=Join-Path $ProjectDir "config/defaults.json"
$AnalysisDir=Join-Path (Join-Path $Dir0 "analysis") $AnalysisProject
...
function Invoke-RStage([string]$Stage){
  ...
  $args=@("--vanilla",$RFile,"--mode=$Stage","--dir0=$Dir0","--analysis_project=$AnalysisProject","--raw_protein_file=$RawProteinFile","--phenotype_rds=$PhenotypeRds","--panel_mapping_file=$PanelMappingFile","--workers=$Workers","--bootstrap_n=$BootstrapN")
...
function Invoke-Train {
  $log=Join-Path $LogDir ("train_{0}.log" -f (Get-Date -Format "yyyyMMdd_HHmmss")); $py=Join-Path $ProjectDir "python/02_train_lightgbm.py"
