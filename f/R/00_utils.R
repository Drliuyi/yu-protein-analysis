...
yuy_abs_path <- function(path, dir0, script_dir = NULL) {
  ...
  root <- if (!is.null(script_dir) && startsWith(path, "config/")) file.path(script_dir, "f") else dir0
  normalizePath(file.path(root, path), winslash = "/", mustWork = FALSE)
}

yuy_init_config <- function(script_dir, cli) {
  cfg <- jsonlite::read_json(file.path(script_dir, "f", "config", "defaults.json"), simplifyVector = TRUE)
...
yuy_print_help <- function() {
  cat(paste0(
    ...
    "Required inputs (Huang-lab defaults under D:/):\n",
    "  data/ukb/phe/Rdata/all.rds\n",
    "  data/ukb/phe/raw/prot_full_unimputed.tsv\n",
    "  data.BIG/gwas/ppp/olink_protein_map_3k_v1.tsv\n\n",
