...
entry_dir <- if (length(file_arg)) dirname(normalizePath(sub("^--file=", "", file_arg[[1]]))) else getwd()
project_dir <- normalizePath(file.path(entry_dir, "..", ".."), winslash = "/", mustWork = TRUE)

source(file.path(project_dir, "f", "R", "00_utils.R"))
source(file.path(project_dir, "f", "R", "01_preflight.R"))
source(file.path(project_dir, "f", "R", "02_prepare_yys.R"))
source(file.path(project_dir, "f", "R", "03_evaluate_report.R"))

cli <- yuy_parse_cli(commandArgs(trailingOnly = TRUE))
cfg <- yuy_init_config(project_dir, cli)
...
if (cfg$mode == "train") {
  stop("Mode=train is dispatched by f/tools/run_yu_yys_windows.ps1 to Python.", call. = FALSE)
}
