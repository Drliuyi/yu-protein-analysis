...
if [[ "${ALLOW_MAC_BULK_PRS_STAGE:-0}" != "1" ]]; then
  printf '%s\n' \
    ...
    "  /mnt/d/scripts/yu-protein-analysis/f/wsl/13_prepare_prs_gwas.sh" \
...
expected="$(($(wc -l < "${PROJECT_DIR}/f/config/prs_gwas_sources.tsv") - 1))"
shopt -s nullglob
...
MAX_P="${MAX_P}" \
RESUME=1 \
bash "${PROJECT_DIR}/f/wsl/13_prepare_prs_gwas.sh" 2>&1 | tee -a "${LOG_FILE}"
