param(
  ...
  [string]$CmrFeatureFile = "D:/analysis/sleepchart_reproduction/data/mribag_features/heart/feature.tsv",
...
$RFile = Join-Path $ProjectDir "f/entry/99_run_yu_full_reproduction.R"
$PythonFile = Join-Path $ProjectDir "f/python/04_full_reproduction.py"
$ConfigFile = Join-Path $ProjectDir "f/config/full_reproduction_defaults.json"
...
function Get-RequestedEndpoints {
  $all = Import-Csv (Join-Path $ProjectDir "f/config/outcomes.csv")
...
function Get-RequestedCmrMetrics {
  $all = Import-Csv (Join-Path $ProjectDir "f/config/cmr_metrics.csv")
