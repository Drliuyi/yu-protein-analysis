...
$RFile = Join-Path $ProjectDir "f/entry/99_run_yu_prs.R"
$FullRFile = Join-Path $ProjectDir "f/entry/99_run_yu_full_reproduction.R"
...
function Invoke-PrsWsl([string]$ScriptName, [string]$GenotypeModeOverride = "") {
  ...
  $scriptWsl = "$projectWsl/f/wsl/$ScriptName"
...
function Get-PrsEndpoints {
  $sourceFile = Join-Path $ProjectDir "f/config/prs_gwas_sources.tsv"
