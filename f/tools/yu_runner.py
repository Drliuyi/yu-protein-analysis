...
class Runner:
    ...
    def resolve_paths(self) -> dict[str, str]:
        legacy = existing_dir("/mnt/d/UKB_data") or existing_dir("D:/UKB_data")
        base = local_path(self.args.dir0 or self.profile.get("dir0", "") or ("D:/UKB_data" if legacy else "D:/"))
    ...
    def doctor(self) -> None:
        ...
        packages = "data.table,R.utils,jsonlite,digest,readxl,survival,pROC,ggplot2,bit64,patchwork,ggrepel,ragg,httr2,curl,igraph,ggalluvial,scales,msigdbr,AnnotationDbi,org.Hs.eg.db,GO.db,TwoSampleMR,CMAverse".split(",")
        ...
        print(f"R packages               {'PASS' if result.returncode == 0 and status == 'PASS' else 'FAIL'}  {status}")
